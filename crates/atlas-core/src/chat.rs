//! RAG (Retrieval-Augmented Generation) integration for `atlas-core`.
//!
...
//! `atlas-vdb`) with LLM inference (via `atlas-llm`) to produce answers
...
use atlas_vdb::{EmbeddingService, InMemoryVectorStore, VectorStore};
...
pub struct ChatOptions {
    ...
    pub embedding_config: Option<atlas_vdb::EmbeddingConfig>,
...
impl ChatOptions {
    ...
    pub fn embedding_config(mut self, config: atlas_vdb::EmbeddingConfig) -> Self {
...
pub fn build_rag_prompt(
    ...
    builder.build().map_err(|e| {
        AtlasError::internal(format!("Failed to build RAG prompt: {e}"))
...
/// that maps `atlas-core` `CodeChunk`s to `ChunkMetadata` entries.
...
pub fn chat_with_context(
    ...
    let response = llm
        ...
        .map_err(|e| {
            AtlasError::internal(format!("LLM inference failed: {e}"))
...
pub fn chat_with_context_stream(
    ...
    embedding_service: &dyn EmbeddingService,
    options: &ChatOptions,
) -> AtlasResult<StreamingChatResponse> {
    let search_start = Instant::now();
    ...
    let rx = llm
        ...
        .map_err(|e| {
            AtlasError::internal(format!("LLM streaming inference failed: {e}"))
...
fn retrieve_context(
    ...
    embedding_service: &dyn EmbeddingService,
    options: &ChatOptions,
) -> AtlasResult<(Vec<String>, Vec<SourceReference>, usize, usize)> {
    ...
    let query_vector = query_vectors.into_iter().next().ok_or_else(|| {
        AtlasError::internal("Embedding service returned no result for query")
            .with_context(ErrorContext::default().with_operation("retrieve_context"))
    })?;
...
/// resolve chunk IDs).
fn load_metadata_store(path: &Path) -> AtlasResult<ChunkMetadataStore> {
    if !path.exists() {
        log::warn!(

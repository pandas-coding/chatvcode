...
use atlas_llm::{ChatTemplate, StopReason, TokenUsage};
use atlas_vdb::{
...
fn setup_vector_store(
    index_result: &atlas_core::IndexResult,
...
fn test_metadata_store_persists_alongside_vector_store() {
    ...
    let _vdb_path = tmp.path().join(".codeatlas/vectors.atvs");
    let metadata_path = tmp.path().join(".codeatlas/chunks.atmd");
...
fn test_source_reference_from_chunk_preserves_metadata() {
    let chunk = CodeChunk {
        ...
        file_path: PathBuf::from("src/main.rs"),
        language: atlas_core::FileLanguage::Rust,
        kind: ChunkKind::Function,
        symbol_name: Some("main".to_string()),
...
fn test_chat_options_builder_chain() {
    let opts = ChatOptions::new("/project")
        ...
        .with_generation_params(atlas_llm::GenerationParams::default().with_max_tokens(1024));
...
fn test_search_uses_metadata_store_without_reindexing() {
    ...
    let vdb_path = tmp.path().join(".codeatlas/vectors.atvs");
    let service = atlas_vdb::MockEmbeddingService::new(32);
    setup_vector_store(&index_result, &service, &vdb_path);

    // Search with metadata store → fast path (no re-indexing)
    let search_opts = atlas_core::SearchOptions::new(mock_embedding_config(), &vdb_path);
...
fn test_full_rag_pipeline_without_llm() {
    ...
    let vdb_path = tmp.path().join(".codeatlas/vectors.atvs");
    let embedding_service = atlas_vdb::MockEmbeddingService::new(32);

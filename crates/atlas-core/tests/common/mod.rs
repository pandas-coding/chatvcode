...
//!
//! Provides a `mock_parse_source` with the same signature as `atlas_parser::parse_source`
//! but implemented with simple pattern matching instead of tree-sitter.
//! This eliminates the cyclic dev-dependency between atlas-core and atlas-parser.

use atlas_core::{
    AtlasResult, ChunkKind, ChunkSpan, CodeChunk, FileLanguage, ParseResult, SourceFile,
};
...
pub fn mock_parse_source(source_file: SourceFile) -> AtlasResult<ParseResult> {
    if !source_file.language.is_supported() {
        let err = atlas_core::AtlasError::unsupported_language(

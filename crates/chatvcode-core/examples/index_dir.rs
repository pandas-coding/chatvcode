use chatvcode_core::{
    ChatVCodeResult, ChunkKind, ChunkSpan, CodeChunk, ParseResult, SourceFile, index_path,
};
use clap::Parser;
use std::path::PathBuf;

#[derive(Parser)]
#[command(name = "index_dir", about = "Index a directory or file with chatvcode")]
struct Args {
    #[arg(default_value = ".")]
    path: PathBuf,
}

fn example_parse_source(source_file: SourceFile) -> ChatVCodeResult<ParseResult> {
...
fn main() {
    ...
    // `chatvcode-core` stays parser-agnostic on purpose, so the example injects
    // a tiny parser instead of depending on `chatvcode-parser` and creating a cycle.

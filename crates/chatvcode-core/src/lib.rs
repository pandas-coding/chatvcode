...
pub use error::{ChatVCodeError, ChatVCodeResult, ErrorContext, ErrorKind, ErrorSeverity};
pub use model::{
...
///
/// This abstraction decouples `chatvcode-core` from `chatvcode-parser`, allowing
...
where
    F: Fn(SourceFile) -> ChatVCodeResult<ParseResult> + Send + Sync,
{
    fn parse(&self, source_file: SourceFile) -> ChatVCodeResult<ParseResult> {
        self(source_file)
    }
...
/// or an [`ChatVCodeError`] if the path is invalid.
pub fn index_path_with_options(
    ...
    if let Some(ref embedding_opts) = options.embedding {
        let embedding_service =
            match chatvcode_vdb::OnnxEmbeddingService::new(embedding_opts.config.clone()) {
                Ok(svc) => svc,
                Err(e) => {
...
pub fn search_with_service(
    ...
    let store = chatvcode_vdb::InMemoryVectorStore::load(&options.vector_store_path).map_err(
        |e: chatvcode_vdb::VdbError| {
            ChatVCodeError::io(format!("Failed to load vector store: {e}"))
                .with_context(
                    ErrorContext::default()
                        .with_operation("search")
...
mod tests {
    ...
    fn mock_parser(source_file: SourceFile) -> ChatVCodeResult<ParseResult> {
        let chunk = CodeChunk {
    ...
    fn test_index_path_collects_parse_errors() {
        ...
        let failing_parser = |_source_file: SourceFile| -> ChatVCodeResult<ParseResult> {
            Err(ChatVCodeError::parse("mock parse failure"))
        };
    ...
    fn test_parallel_index_mixed_success_and_failure() {
        ...
        let selective_parser = |source_file: SourceFile| -> ChatVCodeResult<ParseResult> {
            if source_file.path.to_string_lossy().contains("bad") {
                Err(ChatVCodeError::parse("selective failure"))
            } else {
                let chunk = CodeChunk {
                    id: CodeChunk::generate_id(
                        &source_file.path,

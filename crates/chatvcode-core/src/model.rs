...
///
/// Each variant corresponds to a language that ChatVCode can parse and extract
...
pub struct ParseResult {
    ...
    pub errors: Vec<crate::error::ChatVCodeError>,
}
...
impl ParseResult {
    ...
    #[must_use]
    pub fn with_errors(mut self, errors: Vec<crate::error::ChatVCodeError>) -> Self {
        self.errors = errors;
        self
    }
}
...
impl IndexResult {
    ...
    pub fn from_parse_results(
        ...
        errors: Vec<crate::error::ChatVCodeError>,
    ) -> Self {
...
pub struct EmbeddingOptions {
    ...
    pub config: chatvcode_vdb::EmbeddingConfig,
...
impl IndexState {
    ...
    pub fn load(path: &Path) -> crate::ChatVCodeResult<Self> {
        let text = std::fs::read_to_string(path).map_err(|e| {
            crate::ChatVCodeError::io(format!("Failed to read index state: {e}"))
                .with_context(
        ...
        let state: Self = serde_json::from_str(&text).map_err(|e| {
            crate::ChatVCodeError::internal(format!("Failed to parse index state: {e}"))
                .with_context(
                    crate::ErrorContext::default()
                        .with_operation("load_state")
                        .with_path(path),
    ...
    pub fn save(&self, path: &Path) -> crate::ChatVCodeResult<()> {
        let text = serde_json::to_string_pretty(self).map_err(|e| {
            crate::ChatVCodeError::internal(format!("Failed to serialize index state: {e}"))
                .with_context(
                    crate::ErrorContext::default()
        ...
        if let Some(parent) = path.parent() {
            std::fs::create_dir_all(parent).map_err(|e| {
                crate::ChatVCodeError::io(format!("Failed to create state directory: {e}"))
                    .with_context(
                        crate::ErrorContext::default()
                            .with_operation("save_state")
                            .with_path(parent),
        ...
        std::fs::write(path, text).map_err(|e| {
            crate::ChatVCodeError::io(format!("Failed to write index state: {e}"))
                .with_context(
                    crate::ErrorContext::default()
                        .with_operation("save_state")
                        .with_path(path),
...
pub struct SearchOptions {
    ...
    pub embedding_config: chatvcode_vdb::EmbeddingConfig,
...
impl SearchOptions {
    ...
    pub fn new(
        embedding_config: chatvcode_vdb::EmbeddingConfig,
...
impl ChunkMetadataStore {
    ...
    pub fn load(path: &Path) -> crate::ChatVCodeResult<Self> {
        let text = std::fs::read_to_string(path).map_err(|e| {
            crate::ChatVCodeError::io(format!("Failed to read metadata store: {e}"))
                .with_context(
                    crate::ErrorContext::default()
        ...
        let store: Self = serde_json::from_str(&text).map_err(|e| {
            crate::ChatVCodeError::internal(format!("Failed to parse metadata store: {e}"))
                .with_context(
                    crate::ErrorContext::default()
                        .with_operation("load_metadata_store")
                        .with_path(path),
    ...
    pub fn save(&self, path: &Path) -> crate::ChatVCodeResult<()> {
        let text = serde_json::to_string_pretty(self).map_err(|e| {
            crate::ChatVCodeError::internal(format!("Failed to serialize metadata store: {e}"))
                .with_context(
                    crate::ErrorContext::default()
        ...
        if let Some(parent) = path.parent() {
            std::fs::create_dir_all(parent).map_err(|e| {
                crate::ChatVCodeError::io(format!("Failed to create metadata directory: {e}"))
                    .with_context(
                        crate::ErrorContext::default()
                            .with_operation("save_metadata_store")
                            .with_path(parent),
        ...
        std::fs::write(path, text).map_err(|e| {
            crate::ChatVCodeError::io(format!("Failed to write metadata store: {e}"))
                .with_context(
                    crate::ErrorContext::default()
                        .with_operation("save_metadata_store")
                        .with_path(path),

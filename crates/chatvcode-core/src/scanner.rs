...
impl Scanner {
    ...
    pub fn scan(options: &ScanOptions) -> ScanResult {
        ...
        if !options.root.exists() {
            let err =
                ChatVCodeError::io(format!("Root path does not exist: {}", options.root.display()))
                    .with_context(
    ...
    fn walk_with_gitignore(root: &Path, max_depth: Option<usize>, result: &mut ScanResult) {
        ...
        for entry in walker {
            match entry {
                ...
                Err(e) => {
                    let err = ChatVCodeError::io(format!("Walk error: {e}")).with_context(
                        ErrorContext::default()
                            .with_operation("scan")
                            .with_path(root),
                    );
    ...
    fn walk_directory(
        ...
        let entries = match fs::read_dir(dir) {
            ...
            Err(e) => {
                let err =
                    ChatVCodeError::io(format!("Failed to read directory {}: {e}", dir.display()))
                        .with_context(
                            ErrorContext::default()
                                .with_operation("scan")
                                .with_path(dir),
    ...
    #[must_use]
    pub fn scan_and_read(options: &ScanOptions) -> Vec<ChatVCodeResult<SourceFile>> {
    ...
    fn read_first_n_lines(path: &Path, n: usize) -> ChatVCodeResult<String> {
        let file = fs::File::open(path).map_err(|e| {
            let err = ChatVCodeError::io(format!("Failed to open file {}: {e}", path.display()))
                .with_context(
                    ErrorContext::default()
        ...
        for line in reader.lines().take(n) {
            let line = line.map_err(|e| {
                let err =
                    ChatVCodeError::io(format!("Failed to read line from {}: {e}", path.display()))
                        .with_context(
                            ErrorContext::default()
                                .with_operation("read_first_n_lines")
                                .with_path(path),

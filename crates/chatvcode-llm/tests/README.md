...
```bash
# 运行所有测试（单元测试 + 验收测试）
cargo test -p chatvcode-llm

# 只运行验收测试
cargo test -p chatvcode-llm --test inference_acceptance

# 运行单元测试并显示输出
cargo test -p chatvcode-llm --lib -- --nocapture
```
...
```bash
# 运行集成测试（需要模型文件）
cargo test -p chatvcode-llm --test real_inference -- --ignored
```

### 前置条件

1. 下载 GGUF 模型文件
2. 放置到 `~/.chatvcode/models/` 目录
3. 确保模型文件扩展名为 `.gguf`
...
1. 检查编译错误：`cargo check -p chatvcode-llm`
2. 查看详细输出：`cargo test -p chatvcode-llm -- --nocapture`
3. 运行单个测试：`cargo test -p chatvcode-llm test_name`
...
# 放置到 ~/.chatvcode/models/

# 运行集成测试
cargo test -p chatvcode-llm --test real_inference -- --ignored --nocapture
```
...
```bash
# 生成测试报告（需要 nightly）
cargo +nightly test -p chatvcode-llm -- -Z unstable-options --report-time
```

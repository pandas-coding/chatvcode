...
//!
//! These tests require a GGUF model file in ~/.chatvcode/models/
//! Run with: cargo test -p chatvcode-llm --test embedding -- --ignored

use chatvcode_llm::{LlamaEmbeddingService, LlamaModel};
use std::path::PathBuf;
use std::sync::Arc;

fn get_test_model_path() -> Option<PathBuf> {
    let dir = dirs::home_dir()?.join(".chatvcode").join("models");
    if !dir.exists() {
        return None;
    }
...
///
/// Run with: cargo test -p chatvcode-llm --test embedding -- --ignored test_embedding_service_basic
#[test]
#[ignore]
fn test_embedding_service_basic() {
    let model_path = match get_test_model_path() {
        Some(p) => p,
        None => {
            eprintln!("No GGUF model found in ~/.chatvcode/models/, skipping test");
            return;
        }
    };
...
///
/// Run with: cargo test -p chatvcode-llm --test embedding -- --ignored test_embedding_from_path
#[test]
#[ignore]
...
///
/// Run with: cargo test -p chatvcode-llm --test embedding -- --ignored test_embedding_dimension_matches_model
#[test]
#[ignore]

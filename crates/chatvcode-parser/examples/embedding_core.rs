//! Example: End-to-end embedding + search tests using `chatvcode_parser` + `chatvcode_core`.
//!
//! This example exercises the full embedding pipeline with real tree-sitter
//! parsing, moved here from `chatvcode-core/tests/embedding.rs` to break the cyclic
//! dev-dependency between chatvcode-core and chatvcode-parser.
//!
//! Run: `cargo run --example embedding_core`

use chatvcode_core::{CodeChunk, IndexResult, SearchOptions, index_path, search_with_service};
...
// --- Test functions (copied from chatvcode-core/tests/embedding.rs) ---

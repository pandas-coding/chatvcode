...
///
/// Returns a [`ParseResult`] with chunks and errors, or an [`ChatVCodeError`]
/// if the language is unsupported or parsing fails entirely.
pub fn parse_source(source_file: SourceFile) -> ChatVCodeResult<ParseResult> {
    if !source_file.language.is_supported() {
        let err = ChatVCodeError::unsupported_language("source file language is not supported yet")
            .with_context(
...
fn collect_parse_errors(tree: &Tree, source_file: &SourceFile) -> Vec<ChatVCodeError> {
    let mut errors = Vec::new();
    let mut cursor = tree.walk();

    fn walk(
        cursor: &mut tree_sitter::TreeCursor,
        errors: &mut Vec<ChatVCodeError>,
...
pub fn parser_for_language(language: FileLanguage) -> ChatVCodeResult<ParserService> {
    if language.is_supported() {
        Ok(ParserService::new())
    } else {
        let err = ChatVCodeError::unsupported_language("parser is not available for this language")
            .with_context(
                ErrorContext::default()
